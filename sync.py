...
as treatment entries. Runs on a schedule (every 30 minutes by default).
"""
...
def ns_get_existing_treatments(since_iso):
    """Fetch existing treatments from Nightscout to avoid duplicates."""
...
def ns_upload_treatments(treatments):
    """Upload a list of treatment objects to Nightscout."""
    if not treatments:
        return
...
# ── GLOOKO → NIGHTSCOUT TRANSLATION ──────────────────────────────────────────
def bolus_to_treatment(bolus):
    """Convert a glooko-reader BolusEntry to a Nightscout treatment object."""
    ts = bolus.timestamp
...
def make_dedup_key(treatment):
    """Create a dedup key from timestamp + insulin units."""
...
def sync_once(client):
    ...
    log.info(f"Found {len(boluses)} bolus entries in Glooko")

    if not boluses:
        return 0

    # Fetch existing NS treatments to deduplicate
    try:
        ...
        log.warning(f"Could not fetch existing treatments: {e}. Will upload all.")
        existing_keys = set()

    # Build new treatments, skipping duplicates
    new_treatments = []
